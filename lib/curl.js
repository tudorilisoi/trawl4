...
function fetch(opts) {
    ...
    curl.setOpt(Curl.option.HTTPHEADER, ["Accept-Encoding: gzip"]);
...
// curl.setOpt(Curl.option.URL, '10.255.255.1');
    ...
    curl.on('end', async function (code, responseBody, headers) {
        ...
        for (let hName in lastHeaders) {
            if ('' + hName.toLowerCase() === 'content-encoding') {
                if ('' + lastHeaders[hName].toLowerCase() === 'gzip') {
                    log("*************GZIPPED")
                    try {
                        const unpackedResponseBody = await Q.nfcall(zlib.gunzip, responseBody);
                        _responseBody = unpackedResponseBody
                        console.log(_responseBody);
