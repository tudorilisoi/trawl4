...
class Session {
    ...
    start() {
        ...
        robotsp.then((parser) => {
            ...
            if (
                !this.opts.respawned &&
                [constants.RECRAWL_MODE || constants.CRAWL_MODE].includes(this.opts.mode)
            ) {
    ...
    getNextURI(opts = {}) {
        ...
        return nextP.then(uriModel => {
            ...
            if (passesFilter || uriModel.forced) {
    ...
    filterURI(uri) {
        ...
        let isIncluded = includeArr.length === 0 || util.matchesOne(relURI, includeArr)
        if (!isIncluded) {
            //excludelog('CONF NOT INCLUDED', relURI)
            return null
        }

        let isExcluded = excludeArr.length === 0 ? false : util.matchesOne(relURI, excludeArr)
        if (isExcluded) {
            return null
        }
        return ret
    ...
    processCurl(result) {
        ...
        let uriModel = {
            ...
            httpHeaders: JSON.stringify(result.headers, 2, 2),
    ...
    crawlNextURI(initialUriModel) {
        ...
        nextP.then(uriModel => {
            ...
            if (this.opts.mode === constants.PROCESS_MODE) {
                ...
                if (self.URIcounter > 0 &&
                    self.URIcounter % constants.URL_COUNT_FOR_PROCESS === 0) {
                    Session.exitWithRestartCode()
                }


                ee.emit('EV_CRAWL_COMPLETED', {
                    model: uriModel,
                    session: this,
                })
