...
function getImageURIs(bodyHTML, selector, attr = 'src', baseURI) {
    if (!selector) {
        return []
    }
    ...
    let ret = imageLinks.map(l=>uritools.normalize(l, baseURI)).filter(l=>!!l)
...
class Session {
    ...
    processCurlP(info) {
        ...
        //not text, so no links to find
        if (!util.isTextMime(info.mimeType)) {
            return Q(true)
        }
        ...
        if (this.matchesParser(
            ...
            imageLinks = getImageURIs(info.responseBody, this.imageSelector, baseURI)
            if (imageLinks[0]) {
                normLinks.push(imageLinks[0])
            }
        }
