...
class Session {
    constructor(opts) {
        ...
        log(util.getRuntimePath(opts.domain, '--LRU.json'))
        ...
        ee.on(constants.EV_CRAWL_NEXT, this.getFetchP.bind(this));
    ...
    processCurlP(info) {
        ...
        //log(links, normLinks)
        log(`FOUND ${normLinks.length} LINKS`)
    ...
    getFetchP(initial = false) {
        ...
        if (initial) {
            ...
            nextP = Q(
                ...
                {
                    uri: self.opts.startURI || self.opts.domain
                })
        } else {
        ...
        fetchP.then((result)=> {
            ...
            if (result.skip) {
                log('PARSER EXCLUDE', result.reason, result.uri)
            }
