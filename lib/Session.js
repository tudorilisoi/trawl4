...
class Session {
    ...
    static sleep(seconds = 120) {
        ...
        clearTimeout(this.timeouts['SLEEP'])
        this.timeouts['SLEEP'] = setTimeout(()=> {
            ...
            let crawlP = this.getCurlP(uriModel).then((result)=> {
                ...
                return saveP.then(()=> {
                    ...
                    return pushP.then(()=> {
                        ...
                        log(`SCHEDULE NEXT #${this.URIcounter+1} in ${crawlDelay}ms`)

                        clearTimeout(this.timeouts['CRAWL'])
                        this.timeouts['CRAWL'] = setTimeout(()=> {
                            ee.emit(constants.EV_CRAWL_NEXT)
                        }, crawlDelay)
