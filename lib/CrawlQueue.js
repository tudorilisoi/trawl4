...
class Queue {
    ...
    pushInitial(arr) {
        this.initialURIs = this.initialURIs.concat(arr);
    }
    ...
    next() {
        ...
        if (this.initialURIs.length) {
            const uri = this.initialURIs.pop()
            log('FIND NEXT RETURNING SEED ' + uritools.getRelative(uri))
            return Q({
                uri,
                forced: true,
            });
        ...
        var aggr = {}
        aggr = {limit: 1, order: ['dateLastCrawled', 'A', 'dateAdded', 'A']}
        ...
        if (this.opts.mode === constants.CRAWL_MODE) {
            ...
            filter.state = constants.URI.STATUS_CRAWLED
            aggr.offset = this.offset
        }
